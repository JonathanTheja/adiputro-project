<div>
    <li class="relative" id="sidenav{{ $spk->spk_id }}">
        <a class="flex items-center text-sm py-4 px-6 h-12 overflow-hidden text-gray-700 text-ellipsis whitespace-nowrap rounded hover:text-gray-900 hover:bg-gray-100 transition duration-300 ease-in-out cursor-pointer"
            ...
            data-bs-target="#collapseSidenav{{ $spk->spk_id }}" aria-expanded="true" aria-controls="collapseSidenav{{ $spk->spk_id }}">
    ...
    {{-- <li class="relative">
        <a href="#!"
            ...
            data-mdb-ripple="true" data-mdb-ripple-color="dark">{{ $spk->name }}</a>
    </li> --}}

    @foreach ($spk->children as $sp)

        <ul class="relative accordion-collapse collapse pl-3" id="collapseSidenav{{ $spk->spk_id }}" aria-labelledby="sidenav{{ $spk->spk_id }}"
